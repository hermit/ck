...
function CONFIG(){
	...
	this.window={
		...
		scrollbar_right	:1,
		blink_cursor	:1,
		transparent	:WinTransp.GrassNoEdge,
		...
		alpha_back_colorN	:0x88,
		color_foreground	:0x66000000,
		color_background	:0x66ffffff,
		color_selection		:0x220000ff,
		color_cursor		:0xff00cc00,
		color_imecursor		:0xffcc0000
	};
}
CONFIG.prototype ={
...
function ARGS(args){
	...
	for(var i=1; i < nb; i++){
		var s = args[i];
		if(s=="-fg")	 { if(++i < nb) this.fg = parseInt(args[i], 16);}
...
var Helper ={
	...
	create_shell : function(cfg, workdir){
		if(workdir != null)
			app.CurrentDirectory = workdir;
...
var Commands ={
	...
	wnd_new_shell : function(window){
		...
		if(window != null){
			var pty = window.Pty;
			if(pty != null){
				workdir = pty.CurrentDirectory;
				cfg.copyFrom_pty(pty);
			}
		}
		window.Pty = Helper.create_shell(cfg, workdir);
		return true;
	},
	wnd_new_window : function(window){
		...
		var p = Helper.create_shell(cfg, workdir);
...
Events.base ={
	...
	app_on_new_command : function(_args, workdir){
	...
	app_on_startup : function(_args){
		...
		var p = Helper.create_shell(cfg, null);
		var w = Helper.create_window(cfg);
		w.Pty = p;
		w.Show();
