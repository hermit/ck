...
namespace cygwin{
	...
	inline bool FD_ISET(int n, fd_set* p){
		return (p->fds_bits[n/32] & (1<<(n%32))) ? true : false;
	}
	inline void FD_SET(int n, fd_set* p){
		p->fds_bits[n/32] = (1<<(n%32));
	}
	inline void FD_ZERO(fd_set* p){
		p->fds_bits[0] = p->fds_bits[1] = 0;
	}
	extern "C"{
		...
		__declspec(dllimport) int  select(int, fd_set*, fd_set*, fd_set*, struct timeval*); 
...
static BSTR get_child_current_directory(int cyg_pid){
	...
	static const BYTE code [16] = {
		0x8B,0x44,0x24,0x04,	//mov eax,dword ptr [esp+4];
		0x50,			//push eax;
		0x68,0x04,0x01,0x00,0x00,//push MAX_PATH(260);
		0xFF,0x10,		//call dword ptr[eax];
		0xC2,0x04,0x00,		//ret 4;
		0x90,			//nop;
	};

	static FARPROC pGetCurrentDirectory = 0;
	if(!pGetCurrentDirectory){
		pGetCurrentDirectory = GetProcAddress(GetModuleHandle(L"ntdll.dll"), "RtlGetCurrentDirectory_U");
		if(!pGetCurrentDirectory)
			return 0;
	}
	...
	const DWORD BUFSIZE = sizeof(code)+(sizeof(WCHAR)*261);//MAXPATH+1=261
	BYTE  buffer [BUFSIZE];//16+(2*261)=538bytes

	memcpy(buffer+0, code, sizeof(code));
	*(FARPROC*)(buffer+sizeof(code)) = pGetCurrentDirectory;
	memset(buffer+sizeof(code)+sizeof(FARPROC), 0, (sizeof(WCHAR)*261)-sizeof(FARPROC));

	BYTE* addr = (BYTE*) VirtualAllocEx(process, 0, BUFSIZE, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	if(addr){
		if(WriteProcessMemory(process, addr, buffer, BUFSIZE, 0)){
			HANDLE thread = CreateRemoteThread(process,0,0, (LPTHREAD_START_ROUTINE)addr, addr+sizeof(code), 0,0);
			if(thread){
				if(WaitForSingleObject(thread, INFINITE) == WAIT_OBJECT_0){
					BSTR winpath = SysAllocStringLen(0, 260);
					if(ReadProcessMemory(process, addr+sizeof(code), winpath, sizeof(WCHAR)*260, 0)){
						winpath[260] = '\0';
						size_t len = wcslen(winpath);
						winpath[len] = '\0';
...
UINT CALLBACK Pty_::read_proc(LPVOID lp){
	...
	for(;;){
		...
		while(rawsize < RAWSIZE-512){
			struct cygwin::timeval tv = {0,0};
			cygwin::fd_set fds;
			cygwin::FD_ZERO(&fds);
			cygwin::FD_SET(p->m_fd, &fds);
			n = cygwin::select(p->m_fd+1, &fds, NULL, NULL, &tv);
			if(n!=1)break;
