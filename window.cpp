...
#define MM(a15,a14,a13,a12,a11,a10,a9,a8,a7,a6,a5,a4,a3,a2,a1,a0) \
  {(char)0x##a0,(char)0x##a1,(char)0x##a2, (char)0x##a3, (char)0x##a4, (char)0x##a5, (char)0x##a6, (char)0x##a7,\
   (char)0x##a8,(char)0x##a9,(char)0x##a10,(char)0x##a11,(char)0x##a12,(char)0x##a13,(char)0x##a14,(char)0x##a15};
static const __m128i _cc_alpha255_epi16 = MM(00,FF,00,00,00,00,00,00, 00,FF,00,00,00,00,00,00);
static const __m128i _cc_alpha254_epi16 = MM(00,FE,00,FF,00,FF,00,FF, 00,FE,00,FF,00,FF,00,FF);
static const __m128i _cc_alpha1_epi16   = MM(00,01,00,00,00,00,00,00, 00,01,00,00,00,00,00,00);
static const __m128i _cc_all128_epi16   = MM(00,80,00,80,00,80,00,80, 00,80,00,80,00,80,00,80);
#undef MM

inline __m128i  _cc_div255_epi16(__m128i a){
	__m128i b;
	a = _mm_add_epi16(a, _cc_all128_epi16);
	b = _mm_srli_epi16(a,8);
	a = _mm_add_epi16(a,b);
	a = _mm_srli_epi16(a,8);
	return a;
}

inline UINT  _div255(UINT n){
	n+=128;
	return ((n>>8)+n)>>8;
}

inline void _cc_composite2(DWORD* outbuf, DWORD dst1, DWORD dst2, DWORD src, UINT df1, UINT df2, UINT sf1, UINT sf2, bool invalpha){
	__m128i dcolor,scolor,fact,tmp;

	dcolor = _mm_cvtsi32_si128(dst1);				//0000000000000000 00000000AARRGGBB
	tmp    = _mm_cvtsi32_si128(dst2);				//0000000000000000 00000000aarrggbb
	dcolor = _mm_unpacklo_epi32(dcolor, tmp);			//0000000000000000 AARRGGBBaarrggbb
	dcolor = _mm_unpacklo_epi8(dcolor, _mm_setzero_si128());	//00AA00RR00GG00BB 00aa00rr00gg00bb

	fact = _mm_cvtsi32_si128((0xff<<16)|df1);			//0000000000000000 0000000000ff00DD
	tmp  = _mm_cvtsi32_si128((0xff<<16)|df2);			//0000000000000000 0000000000ff00dd
	fact = _mm_unpacklo_epi64(fact, tmp);				//0000000000ff00DD 0000000000ff00dd
	fact = _mm_shufflehi_epi16(fact, _MM_SHUFFLE(1,0,0,0));		//00ff00DD00DD00DD 0000000000ff00dd
	fact = _mm_shufflelo_epi16(fact, _MM_SHUFFLE(1,0,0,0));		//00ff00DD00DD00DD 00ff00dd00dd00dd
	dcolor = _mm_mullo_epi16(dcolor, fact);//

	scolor = _mm_cvtsi32_si128(src);				//0000000000000000 00000000aarrggbb
	scolor = _mm_unpacklo_epi32(scolor, scolor);			//0000000000000000 aarrggbbaarrggbb
	scolor = _mm_unpacklo_epi8(scolor, _mm_setzero_si128());	//00aa00rr00gg00bb 00aa00rr00gg00bb

	fact = _mm_cvtsi32_si128(sf1);					//0000000000000000 00000000000000SS
	tmp  = _mm_cvtsi32_si128(sf2);					//0000000000000000 00000000000000ss
	fact = _mm_unpacklo_epi64(fact, tmp);				//00000000000000SS 00000000000000ss
	fact = _mm_shufflehi_epi16(fact, _MM_SHUFFLE(1,0,0,0));		//000000SS00SS00SS 00000000000000ss
	fact = _mm_shufflelo_epi16(fact, _MM_SHUFFLE(1,0,0,0));		//000000SS00SS00SS 000000ss00ss00ss
	scolor = _mm_mullo_epi16(scolor, fact);//

	dcolor = _mm_add_epi16(dcolor, scolor);
	dcolor = _cc_div255_epi16(dcolor);

	if(invalpha){
		dcolor = _mm_xor_si128(dcolor, _cc_alpha255_epi16);
		dcolor = _mm_or_si128(dcolor, _cc_alpha1_epi16);
	}

	dcolor = _mm_packus_epi16(dcolor, dcolor);			//AARRGGBBaarrggbb AARRGGBBaarrggbb
	_mm_storel_epi64((__m128i*)outbuf, dcolor);
}

inline DWORD _cc_composite1(DWORD dst, DWORD src, UINT df, UINT sf, bool invalpha){
	__m128i dcolor,scolor,fact;

	dcolor = _mm_cvtsi32_si128(dst);				//0000000000000000 00000000AARRGGBB
	dcolor = _mm_unpacklo_epi8(dcolor, _mm_setzero_si128());	//0000000000000000 00AA00RR00GG00BB

	fact = _mm_cvtsi32_si128((0xff<<16)|df);			//0000000000000000 0000000000ff00dd
	fact = _mm_shufflelo_epi16(fact, _MM_SHUFFLE(1,0,0,0));		//0000000000000000 00ff00dd00dd00dd
	dcolor = _mm_mullo_epi16(dcolor, fact);//

	scolor = _mm_cvtsi32_si128(src);				//0000000000000000 00000000aarrggbb
	scolor = _mm_unpacklo_epi8(scolor, _mm_setzero_si128());	//0000000000000000 00aa00rr00gg00bb

	fact = _mm_cvtsi32_si128(sf);					//0000000000000000 00000000000000ss
	fact = _mm_shufflelo_epi16(fact, _MM_SHUFFLE(1,0,0,0));		//0000000000000000 000000ss00ss00ss
	scolor = _mm_mullo_epi16(scolor, fact);//
	...
	dcolor = _mm_packus_epi16(dcolor, dcolor);			//0000000000000000 AARRGGBBAARRGGBB
	return _mm_cvtsi128_si32(dcolor);
}

inline void composite_argb2_to_pargb(DWORD* outbuf, DWORD dst1, DWORD dst2, DWORD src){
	UINT sa, da1,da2, df1,df2;
	sa  = (src>>24) & 0xff;

	da1 = (dst1>>24) & 0xff;
	df1 = _div255(da1 * (255-sa));
	da1 = df1 + sa;

	da2 = (dst2>>24) & 0xff;
	df2 = _div255(da2 * (255-sa));
	da2 = df2 + sa;

	_cc_composite2(outbuf, (da1<<24)|(dst1&0x00ffffff), (da2<<24)|(dst2&0x00ffffff), src, df1,df2, sa,sa, false);
}

inline void composite_argb1_to_pargb(DWORD* outbuf, DWORD dst, DWORD src){
	UINT sa,da,df;
	sa = (src>>24) & 0xff;

	da = (dst>>24) & 0xff;
	df = _div255(da * (255-sa));
	da = df + sa;

	outbuf[0] = _cc_composite1((da<<24)|(dst&0x00ffffff), src, df,sa, false);
}

inline void composite_argb2_inv(DWORD* outbuf, DWORD dst1, DWORD dst2, DWORD src){
	UINT sa, da1,da2, sf1,sf2;
	sa  = (src>>24) & 0xff;

	da1 = (dst1>>24) & 0xff;
	da1 = _div255(da1 * (255-sa)) + sa;
	sf1 = (da1) ? int(sa * (255.0f/da1)) : 0;

	da2 = (dst2>>24) & 0xff;
	da2 = _div255(da2 * (255-sa)) + sa;
	sf2 = (da2) ? int(sa * (255.0f/da2)) : 0;

	_cc_composite2(outbuf, (da1<<24)|(dst1&0x00ffffff), (da2<<24)|(dst2&0x00ffffff), src, 255-sf1, 255-sf2, sf1, sf2, true);
}
inline void composite_argb1_inv(DWORD* outbuf, DWORD dst, DWORD src){
	UINT sa,da,sf;
	sa = (src>>24) & 0xff;

	da = (dst>>24) & 0xff;
	da = _div255(da * (255-sa)) + sa;
	sf = (da) ? int(sa * (255.0f/da)) : 0;

	outbuf[0] = _cc_composite1((da<<24)|(dst&0x00ffffff), src, 255-sf, sf, true);
}

inline DWORD composite_argb(DWORD dst, DWORD src){
	#if 1
	UINT sa,da,sf;
	sa = (src>>24) & 0xff;
	...
	return _cc_composite1((da<<24)|(dst&0x00ffffff), src, 255-sf, sf, false);
	#else
	UINT da,dr,dg,db, sa,sr,sg,sb, df,sf;
	da = (dst>>24) & 0xff;
	dr = (dst>>16) & 0xff;
	dg = (dst>>8 ) & 0xff;
	db = (dst>>0 ) & 0xff;
	...
	dr = _div255( (dr * df) + (sr * sf) );
	dg = _div255( (dg * df) + (sg * sf) );
	db = _div255( (db * df) + (sb * sf) );

	return (da<<24) | (dr<<16) | (dg<<8) | (db);
	#endif
}

inline void to_pargb2_inv(DWORD* iobuf){
	__m128i b, a = _mm_loadl_epi64((__m128i const*)iobuf);
	a = _mm_unpacklo_epi8(a, _mm_setzero_si128());
	a = _mm_xor_si128(a, _cc_alpha255_epi16);//invert

	b = _mm_shufflelo_epi16(a, _MM_SHUFFLE(3,3,3,3));
	b = _mm_shufflehi_epi16(b, _MM_SHUFFLE(3,3,3,3));
	b = _mm_or_si128(b, _cc_alpha255_epi16);

	a = _mm_mullo_epi16(a,b);
	a = _cc_div255_epi16(a);

	a = _mm_packus_epi16(a,a);
	_mm_storel_epi64((__m128i*)iobuf, a);
}

inline void to_pargb1_inv(DWORD* iobuf){
	__m128i b, a = _mm_cvtsi32_si128(iobuf[0]);
	a = _mm_unpacklo_epi8(a, _mm_setzero_si128());
	a = _mm_xor_si128(a, _cc_alpha255_epi16);//invert
	...
	a = _mm_packus_epi16(a,a);
	iobuf[0] = _mm_cvtsi128_si32(a);
}

inline DWORD to_pargb(DWORD n){
	#if 1
	__m128i b, a = _mm_cvtsi32_si128(n);
	a = _mm_unpacklo_epi8(a, _mm_setzero_si128());
...
public:
	...
	void Fill(DWORD argb){
		...
		if(m_bg_x == 0){
			...
			int rcw_odd = (rc.right - rc.left) & 1;
			DWORD* dst_ptr = m_pixels + (m_width * rc.top);
			for(int y=rc.top; y < rc.bottom; y++){
				DWORD* bg_ptr = m_bg->pixels() + (m_bg->width() * m_bg_y[y]);
				int x = rc.left;
				for(; x < rc.right; x+=2)
					composite_argb2_to_pargb(dst_ptr+x, bg_ptr[m_bg_x[x+0]], bg_ptr[m_bg_x[x+1]], argb);
				if(rcw_odd)
					composite_argb1_to_pargb(dst_ptr+x, bg_ptr[m_bg_x[x+0]], argb);
				dst_ptr += m_width;
			}
		}
	}

	void Text_fill(RECT& rc, DWORD argb){
		if(m_bg_x == 0){
			...
			for(int y=rc.top; y < rc.bottom; y++){
				...
				for(; x < rc.right-1; x+=2)
					composite_argb2_inv(dst_ptr+x, bg_ptr[m_bg_x[x+0]], bg_ptr[m_bg_x[x+1]], argb);
				if(rcw_odd)
					composite_argb1_inv(dst_ptr+x, bg_ptr[m_bg_x[x+0]], argb);
				dst_ptr += m_width;
			}
		}
	}
	void Text_rect(RECT& rc, DWORD argb){
		if(m_bg_x == 0){
			argb = argb ^ 0xFF000000 | 0x01000000;
			DWORD* dst_ptr = m_pixels + (m_width * rc.top);
			for(int x=rc.left; x < rc.right; x++)
			...
			DWORD* bg_ptr;
			{
				...
				for(int x=rc.left; x < rc.right; x++)
					composite_argb1_inv(dst_ptr+x, bg_ptr[m_bg_x[x]], argb);
				dst_ptr += m_width;
			}
			for(int y=rc.top+1; y < rc.bottom-1; y++){
				bg_ptr = m_bg->pixels() + (m_bg->width() * m_bg_y[y]);
				composite_argb1_inv(dst_ptr+rc.left   , bg_ptr[m_bg_x[rc.left   ]], argb);
				composite_argb1_inv(dst_ptr+rc.right-1, bg_ptr[m_bg_x[rc.right-1]], argb);
				dst_ptr += m_width;
			}
			{
	...
	#define AddAlpha(AL,PX) \
		if( !((AL) & MASK) && (PX & MASK) ){\
			PX = ( ((PX>>24) < ALPHA) ? (PX & 0x00ffffff) : (PX-(ALPHA<<24)) ) | MASK; \
		}
	void Text_glow(RECT& rc, BYTE alpha){
		const DWORD MASK = 0x01000000;
		const DWORD ALPHA = alpha;
	...
	void Text_to_pargb(RECT& rc){
		int rcw_odd = (rc.right - rc.left) & 1;
		DWORD* dst_ptr = m_pixels + (m_width * rc.top);
		for(int y=rc.top; y < rc.bottom; y++){
			int x = rc.left;
			for(; x < rc.right-1; x+=2)
				to_pargb2_inv(dst_ptr+x);
			if(rcw_odd)
				to_pargb1_inv(dst_ptr+x);
			dst_ptr += m_width;
		}
	}
};
...
protected:
	...
	LRESULT wm_on_message(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp){
		...
		case WM_KEYDOWN:
			...
			return 0;
		case WM_MOUSEWHEEL:
			m_notify->OnMouseWheel(GET_WHEEL_DELTA_WPARAM(wp) / WHEEL_DELTA);
			return 0;
