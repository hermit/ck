...
protected:
	...
	LRESULT app_on_new_command_op(DWORD pid, void* lp){
		...
		if(process){
			struct{ DWORD  u0,u1; LPCWSTR p0,p1; } d;
			DWORD n;
			if(ReadProcessMemory(process, lp, &d, sizeof(d), &n) &&
			   d.u0 < 32768 && d.u1 < 32768){//max32kB
				BSTR  cmdline = SysAllocStringLen(0, d.u0);
				BSTR  workdir = SysAllocStringLen(0, d.u1);
				if(cmdline && workdir &&
				   ReadProcessMemory(process, d.p0, cmdline, sizeof(WCHAR) * d.u0, &n) &&
				   ReadProcessMemory(process, d.p1, workdir, sizeof(WCHAR) * d.u1, &n)){
					((UINT*)cmdline)[-1] = sizeof(WCHAR) * d.u0;
					((UINT*)workdir)[-1] = sizeof(WCHAR) * d.u1;
					cmdline[d.u0] = '\0';
					workdir[d.u1] = '\0';
